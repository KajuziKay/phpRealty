...
class CurrencyFormatter 
{
	...
	function toDecimalForStyle( $amount, $style ) 
	{
		...
		$rv = ereg_replace( $thousandsSeparator, "", $amount );
		$rv = ereg_replace( $decimalPlace, ".", $rv );
		$rv = $rv + 0;
		return $rv;
	}
	...
	function getSymbol( $ISOCode ) 
	{
		...
		if(!in_array(strtoupper($ISOCode),$this->nonDollarSymbols))
		{
			$symbol = "\$";
		}else{
			$symbol = $this->nonDollarSymbol[strtoupper($ISOCode)];
		}
